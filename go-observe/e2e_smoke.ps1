...
Set-Location $here
go build -o (Join-Path $env:TEMP 'observe-dsl.exe') ./cmd/observe-dsl
Write-Host '=== seed ==='
& (Join-Path $env:TEMP 'observe-dsl.exe') --project-root $proj seed
Write-Host '=== loop（默认探测 ledger）==='
& (Join-Path $env:TEMP 'observe-dsl.exe') --project-root $proj loop "$proj\events.jsonl"
Write-Host '=== proposals ==='
& (Join-Path $env:TEMP 'observe-dsl.exe') --project-root $proj proposals
Write-Host '=== dsl.json ==='
Get-Content (Join-Path $proj '.agent\observe\dsl.json') -Raw
