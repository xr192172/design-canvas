...
### 版本升级契约差检测 & 局部重写（CLI，非 MCP 工具）

老项目重构的依赖治理：不同子项目可能声明不同运行时版本（如 JDK 8 与 JDK 21 并存），只需把**契约对不上的那部分语言特性**重写即可。两个 CLI 承担「检测 + 闭环改写」：

```bash
# ① 契约差检测（只报告，不改写）——工具链盘点 + 语言特性超标 + 废弃/移除 API
npm run upgrade-scan -- <项目目录>            # 人类可读报告
npm run upgrade-scan -- <项目目录> --json     # 结构化输出（供 LLM/闭环消费）

# ② 局部重写闭环——差异报告 → 重写建议 → 应用 → 验证 → 提交/回退
npm run upgrade-rewrite -- <项目目录>                     # 仅报告模式
npm run upgrade-rewrite -- <项目目录> --apply <edits.json> # 闭环模式
```

| 参数 | 说明 |
|---|---|
| `<项目目录>` | 多子项目仓库根目录（自动识别嵌套子项目，跳过重复扫描） |
| `--json` | 结构化输出工具链 / 特性超标 / 废弃 API 命中 |
| `--apply <edits.json>` | 进入闭环：应用编辑（`[{"file","from","to"}]`，`from` 必须唯一命中，歧义/未命中拒绝） |
| `--skip-verify` | 跳过验证阶段（未验证的改动不自动提交） |

检测三阶段：**A 工具链版本盘点**（扫描 `pom.xml` / `build.gradle` / `.nvmrc` / `.tool-versions` / `go.mod` / `engines`，探测本机 java/node/go，输出 ok/missing/mismatch）→ **B 语言特性契约差**（以子项目声明版本为边界，报告超出版本的语言特性如 `var` / switch 表达式 / record / 文本块，附行号与重写建议）→ **C 废弃/移除 API**（如 JDK 11 起移除的 JAXB，Node 的 `new Buffer`，附替代方案）。

闭环安全网（复用既有）：改前基线自动提交 → 应用前校验（歧义/未命中拒绝，一个文件都不改）→ 按项目形态探测验证命令（`tsc --noEmit`+`npm test` / `go build`+`go test`）→ 验证通过精确提交、失败 git 还原到改写前。
