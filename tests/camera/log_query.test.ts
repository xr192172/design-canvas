...
import { queryCameraLog } from '../../src/camera/log_query.js';

function writeEvents(file: string, lines: string[]): void {
  fs.writeFileSync(file, lines.join('\n') + '\n', 'utf8');
}

describe('Camera 日志查询 · queryCameraLog', () => {
  let dir: string;
  let eventsPath: string;

  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'camera-log-test-'));
  ...
  it('事件文件不存在 → 空结果', () => {
    const r = queryCameraLog(path.join(dir, 'nope.jsonl'));
    expect(r.total).toBe(0);
    expect(r.entries).toEqual([]);
  });

  it('按文件过滤：只返回该文件事件，异常判定正确', () => {
    const r = queryCameraLog(eventsPath, { files: ['src/a.ts'] });
  ...
  it('文件名片段过滤（b.ts）不串文件', () => {
    const r = queryCameraLog(eventsPath, { files: ['b.ts'] });
    expect(r.total).toBe(1);
    expect(r.entries[0].probe).toBe('b.enter');
  });

  it('未指定文件默认只列偏差', () => {
    const r = queryCameraLog(eventsPath);
  ...
  it('未指定文件且 all=true → 全量', () => {
    const r = queryCameraLog(eventsPath, { all: true });
