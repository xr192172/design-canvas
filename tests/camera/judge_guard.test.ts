/**
 * Camera 实时判定守卫测试：verify serve 内嵌开关 CAMERA_GUARD 的行为。
 *
 * 验证点：
 *   1. 默认关闭（CAMERA_GUARD 未设/非 1）时 judgeGuardLog 是纯 no-op。
 *   2. 开启后，偏差事件打印到 console.error，正常事件不打印。
 *   3. 开启后打印内容包含 rule/probe。
 */
import { describe, it, expect, afterEach, vi } from 'vitest';
import { judgeGuardLog, judgeGuardEnabled } from '../../src/camera/judge_guard.js';
...
afterEach(() => {
  vi.restoreAllMocks();
  delete process.env.CAMERA_GUARD;
});

describe('Camera 实时判定守卫 · 开关', () => {
  it('默认关闭：CAMERA_GUARD 未设置时 judgeGuardEnabled 为 false', () => {
    expect(judgeGuardEnabled()).toBe(false);
  });

  it('CAMERA_GUARD=1 时开启', () => {
    process.env.CAMERA_GUARD = '1';
    expect(judgeGuardEnabled()).toBe(true);
  });

  it('CAMERA_GUARD=0 时关闭', () => {
    process.env.CAMERA_GUARD = '0';
    expect(judgeGuardEnabled()).toBe(false);
  });
});

describe('Camera 实时判定守卫 · 打印', () => {
  it('关闭时不打印任何偏差', () => {
  ...
  it('开启后偏差事件打印，含 rule 与 probe', () => {
    ...
    expect(text).toContain('camera-guard');
