/**
 * Camera 工具注册测试：验证 camera_log / camera_judge / camera_instrument
 * 已注册进同一套 MCP server_registry，且行为正确（复用 camera/* 纯函数）。
 *
 * 验证点：
 *   1. 三个 camera_* 工具都在 TOOL_DEFS 中（与 design 主工具并列）。
 *   2. camera_judge：对一批事件判定，返回 total/ok/deviation 汇总。
 *   3. camera_judge：缺 events 参数报错。
 *   4. camera_log：缺 events_file 报错；给不存在文件返回空结果。
 *   5. camera_log：按文件过滤 + all 语义。
 *   6. camera_instrument：dry-run 不写盘；缺 target 报错。
 */
...
import { TSProbeCapture } from '../../src/camera/probe';
...
describe('Camera 工具注册', () => {
  it('camera_log/camera_judge/camera_instrument 都在 TOOL_DEFS 中', () => {
    expect(TOOL_DEFS.some((d) => d.name === 'camera_log')).toBe(true);
    expect(TOOL_DEFS.some((d) => d.name === 'camera_judge')).toBe(true);
    expect(TOOL_DEFS.some((d) => d.name === 'camera_instrument')).toBe(true);
  });
});

describe('camera_judge', () => {
  it('对事件数组判定，返回汇总', async () => {
    const r = await handlerOf('camera_judge')({
      events: [
...
describe('camera_log', () => {
  it('缺 events_file 报错', async () => {
    const r = await handlerOf('camera_log')({});
    expect(r.isError).toBe(true);
    expect(r.text).toContain('events_file');
  });

  it('不传 files 默认只列偏差', async () => {
    const r = await handlerOf('camera_log')({ events_file: eventsFile });
    expect(r.isError).toBeFalsy();
  ...
  it('all=true 全量列出，含文件路径', async () => {
    const r = await handlerOf('camera_log')({ events_file: eventsFile, all: true });
    expect(r.isError).toBeFalsy();
  ...
  it('按文件过滤只返回命中路径', async () => {
    const r = await handlerOf('camera_log')({ events_file: eventsFile, files: ['src/b.ts'] });
    expect(r.isError).toBeFalsy();
...
describe('camera_instrument', () => {
  it('缺 target 报错', async () => {
    const r = await handlerOf('camera_instrument')({});
    expect(r.isError).toBe(true);
  ...
  it('dry-run 预览不写盘', async () => {
    ...
    const r = await handlerOf('camera_instrument')({ action: 'instrument', target: proj, dry_run: true });
    expect(r.isError).toBeFalsy();
