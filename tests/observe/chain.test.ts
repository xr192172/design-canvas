/**
 * P2 链路重建与链路契约判定测试（TS 侧，对齐 go-observe/probe/chain_test.go 语义）
 *
 * 覆盖：
 ...
 *   - normalizeEvents 保留 v2 trace 字段（observe_judge 链路判定的数据通路）
 */
...
describe('normalizeEvents 保留 v2 trace 字段', () => {
  it('trace_id/frame_id 原样透传（observe_judge 链路判定的数据通路）', () => {
