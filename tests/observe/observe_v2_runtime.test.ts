/**
 * v2 分级采集 runtime 测试（对齐 go-observe/probe 测试集）。
 *
 * 覆盖 observe_v2 决策卡验收：
...
import { Tiered, setGlobalTiered, isCatchProbe } from '../../src/observe/tiered.js';
import { withScope, currentScope, currentTraceId } from '../../src/observe/trace.js';
import { ExportGate, splitEvents } from '../../src/observe/export_incident.js';
import { TSProbeCapture, type TSEvent } from '../../src/observe/probe.js';

function tmpDir(prefix = 'observe-v2-'): string {
...
describe('c8 导出限流门', () => {
  ...
  it('catch 风暴下 incident 文件数 ≤ 限额', async () => {
    const dir = tmpDir('observe-storm-');
...
describe('全量镜像 + 全局路由', () => {
  it('fullSink 镜像每事件，setGlobalTiered 后 enterScope 免传 tiered', async () => {
    const dir = tmpDir('observe-mirror-');
