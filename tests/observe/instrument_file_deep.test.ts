/**
 * Observe deep 级插桩测试（PR 偏差1）：验证 enableDeep 时捕获函数内部数据流动。
 *
 * 验证点：
 ...
 *   4. deep 幂等：二次 enableDeep 插桩跳过（observe:deep 标记）。
 *   5. 注入不破坏语法（可安全重求值）。
 */
...
describe('deep 级插桩', () => {
  it('enableDeep=true 时捕获变量赋值与条件分支（level:deep）', async () => {
    ...
    expect(content).toContain('observe:deep');
  ...
  it('enableDeep 默认 false 时不注入 deep 探针', async () => {
    ...
    const content = fs.readFileSync(file, 'utf-8');
    expect(content).not.toContain('observe:deep');
