...
import { chainRecon } from '../../src/tools/chain_recon';
import { saveDSL } from '../../src/storage';

describe('chain_recon e2e（中观档：派生 → 缓存 → 事件对账）', () => {
  it('对真实源文件 camera/chain.ts 跑通全管线', async () => {
    // 自建临时 feature：宿主节点指向真实源文件（绝对路径）
    const feature = 'cr_e2e';
    const nodeId = 'file_chain_ts';
    const realFile = path.resolve('src/camera/chain.ts');
    saveDSL({
      feature,
      title: 'chain_recon e2e',
    ...
    const projectDir = process.cwd(); // 无 .agent/camera → 空事件，验证 not_run

    // 阶段1：无事件 → 自动派生（后工具自动前置），not_run 诚实标注
    const r1 = await chainRecon({ feature, node_id: nodeId, project_dir: projectDir });
    ...
    // 阶段2：再跑一次 → 命中缓存跳过派生
    const r2 = await chainRecon({ feature, node_id: nodeId, project_dir: projectDir });
    ...
    const events = want.map((n, i) => ({
      trace_id: 't1',
      frame_id: i + 1,
      probe: 'camera.chain.' + n,
      time: new Date(Date.now() + i * 1000).toISOString(),
      fields: {
        file: 'src/camera/chain.ts',
    ...
    const r3 = await chainRecon({ feature, node_id: nodeId, project_dir: projectDir, events_files: [ef] });
