/**
 * chain_recon —— 中观档工具：按「文件 / 宿主节点」一条命令的真跑 + 查数据 + 对账
 *
 * 定位（design-canvas 工具可用性复盘 2026-08-26，缺口桶 C）：
 *   宏观对账（整项目 contract vs 全量 camera 事件）重，微观（trace-exec 纯函数
 *   子集，一碰真实 I/O 就 unsupported）假——两者之间空着的「按这一条链真跑 +
 *   查这条链真数据 + 对该块」就是本工具。
 *
 * 编排（后工具自动前置 + 缓存跳过）：
 *   1. deriveDetailChain（建链）—— 宿主下已有 detail 链则缓存命中跳过，没有才自动派生
 *   2. 自动发现被观测项目的事件文件（.agent/camera + .design-canvas/camera）
 *   3. queryCameraLog 按链涉及文件过滤 → 这条链的真跑事件
 *   4. judgeEvent 逐事件判定 → 偏差清单（silent-error-discard 等）
 *   5. rebuildChains 从带 trace 的事件重建实测调用链
 *   6. 链路契约匹配：声明链 vs 实测链（子序列语义，与 camera_chain_view 一致）
 *
 * 诚实纪律：工具不产生事实、不伪造事件。该链无任何事件 → not_run=true，明示
 * 「先跑一遍再对账」，绝不降级冒充成品。
 *
 * 探针名近似：camera 事件探针名是 base probe（如 svc.Handle），detail 链是函数名
...
import { rebuildChains, matchChainDecl, type TSChainObs } from '../camera/chain.js';
import { loadTSEvents, type TSEvent } from '../camera/probe.js';

export interface ChainReconInput {
  /** DSL feature 名 */
  feature: string;
...
export interface ChainReconFunction {
  step: number;
  name: string;
  signature?: string;
  node_id: string;
...
export interface ChainReconObserved {
  trace_id: string;
  sequence: string[];
  errs: number;
  events: number;
...
export interface ChainReconDeviation {
  probe: string;
  time: string;
  rule: string;
  reason: string;
...
export interface ChainReconResult {
  feature: string;
  node_id: string;
  host_file: string;
  /** 本次建链方式：derived=新派生；cached=命中缓存复用 */
  derive: 'derived' | 'cached';
  chain: ChainReconFunction[];
  events: {
    files: string[];
    chain_events: number;
    total_events: number;
  };
  observed: ChainReconObserved[];
  deviations: ChainReconDeviation[];
  chain_match: {
    mode: 'bare-name';
...
/** 自动发现事件文件：.agent/camera + .design-canvas/camera（与 reconcile_effects 对齐） */
...
function readDerivedChain(
  ...
  nodeId: string,
  projectRoot: string,
): { chain: ChainReconFunction[]; hostFile: string | null } {
  ...
  const chain: ChainReconFunction[] = [];
  nodes.forEach((n, i) => {
...
export async function chainRecon(input: ChainReconInput): Promise<ChainReconResult> {
  ...
  // ── 后工具自动前置：建链（缓存命中跳过派生）──
  let derive: ChainReconResult['derive'] = 'cached';
  ...
  for (const ef of eventFiles) {
    ...
    const q = queryCameraLog(ef, { files: chainFiles, all: true });
    totalEvents += q.total;
    // 命中：重新 load 该文件中命目录入事件（queryCameraLog 只给判定条目，缺 trace 细节）
    if (q.total > 0) {
      hitFiles.push(ef);
  ...
  // ── 逐事件判定（偏差清单）──
  const deviations: ChainReconDeviation[] = chainEvents
    .map((ev) => {
    ...
    .filter((d): d is ChainReconDeviation => d !== null);

  // ── 重建实测调用链 + 链路契约匹配 ──
  let observed: ChainReconObserved[] = [];
  let chainStatus: ChainReconResult['chain_match']['status'] = 'no-events';
  let matched = 0;
  ...
  // ── 人类可读报告 ──
  const lines = [
    `chain_recon 中观档 · feature "${feature}" · 宿主 ${node_id}`,
