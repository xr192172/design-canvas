...
(function() {
  ...
  function setupExport() {
    ...
    const msgFlowBtn = document.getElementById('msg-flow-toggle');
    if (msgFlowBtn) {
      msgFlowBtn.addEventListener('click', () => {
        if (window.toggleMsgFlowPanel) window.toggleMsgFlowPanel();
      });
    }
  ...
  function setupSimulation() {
    ...
    window.toggleMsgFlowPanel = function() { toggleMsgFlowPanel(); };
    let trace = [];
    let step = 0;

    // ==== 创建 Section 卡片容器（重写：干净布局，无递归 resize） ====
    var sectionCardLayout = { cx: 0, cy: 0, cw: 0, ch: 0, maxH: 0 };

    function createSectionCardContainer() {
      var sqNode = document.querySelector('.node[data-id="node_section_queue"]');
      if (!sqNode) return;
      var sqRect = sqNode.querySelector('rect[data-shape="true"]');
      if (!sqRect) return;

      var sqX = parseFloat(sqRect.getAttribute('x')) || 0;
      var sqY = parseFloat(sqRect.getAttribute('y')) || 0;
      var sqW = parseFloat(sqRect.getAttribute('width')) || 900;
      var sqH = parseFloat(sqRect.getAttribute('height')) || 150;

      // 1. 隐藏静态子节点（被动画卡片替代）
      var staticChildIds = ['node_section_active', 'node_section_folded', 'node_section_corpus', 'node_rollback'];
      staticChildIds.forEach(function(id) {
        var el = document.querySelector('.node[data-id="' + id + '"]');
        if (el) el.style.display = 'none';
        // 同时隐藏对应的 contains 边
        var edgeEl = document.querySelector('.edge[data-from="node_section_queue"][data-to="' + id + '"]');
        if (edgeEl) edgeEl.style.display = 'none';
      });

      // 2. 扩展 SectionQueue 高度，为卡片腾出空间
      var targetH = 380;
      var dh = targetH - sqH;
      sqRect.setAttribute('height', targetH);
      if (nodeById['node_section_queue']) {
        nodeById['node_section_queue'].height = targetH;
      }

      // 3. 下移 SectionQueue 之后的所有节点，避免重叠
      var pushDownIds = [
        'node_current_round', 'node_round_messages', 'node_draft_zone',
        'node_draft_add', 'node_draft_budget', 'node_round_advance',
        'node_dynamic_injection', 'node_retrieval_section', 'node_retrieval_corpus',
        'node_retrieval_graph', 'node_retrieval_skill', 'node_retrieval_cache',
        'node_context_compose', 'node_compose_order', 'node_compose_token',
        'node_llm_aam'
      ];
      pushDownIds.forEach(function(id) {
        var el = document.querySelector('.node[data-id="' + id + '"]');
        if (!el) return;
        var r = el.querySelector('rect[data-shape="true"]');
        if (r) {
          var y = parseFloat(r.getAttribute('y'));
          r.setAttribute('y', y + dh);
        }
        el.querySelectorAll('text').forEach(function(t) {
          var ty = parseFloat(t.getAttribute('y'));
          if (!isNaN(ty)) t.setAttribute('y', ty + dh);
        });
        el.querySelectorAll('line').forEach(function(l) {
          var ly1 = parseFloat(l.getAttribute('y1'));
          var ly2 = parseFloat(l.getAttribute('y2'));
          if (!isNaN(ly1)) l.setAttribute('y1', ly1 + dh);
          if (!isNaN(ly2)) l.setAttribute('y2', ly2 + dh);
        });
        if (nodeById[id]) {
          nodeById[id].y = (nodeById[id].y || 0) + dh;
        }
      });

      // 4. 更新受影响的边
      updateConnectedEdges('node_section_queue');
      pushDownIds.forEach(function(id) { updateConnectedEdges(id); });
      ...
      sectionCardLayout = { cx: cx, cy: cy, cw: cw, ch: ch, maxH: ch };

      var ns = 'http://www.w3.org/2000/svg';
      var containerNode = document.createElementNS(ns, 'g');
      containerNode.setAttribute('class', 'node section-card-container');
      containerNode.setAttribute('data-id', 'node_section_card_container');

      var containerRect = document.createElementNS(ns, 'rect');
      containerRect.setAttribute('x', cx);
      containerRect.setAttribute('y', cy);
      containerRect.setAttribute('width', cw);
      containerRect.setAttribute('height', ch);
      containerRect.setAttribute('rx', 4);
      containerRect.setAttribute('fill', '#0d1b2a');
      containerRect.setAttribute('stroke', '#4a6fa5');
      containerRect.setAttribute('stroke-width', 1);
      containerRect.setAttribute('stroke-dasharray', '4 2');
      containerRect.setAttribute('data-shape', 'true');
      containerNode.appendChild(containerRect);

      var containerLabel = document.createElementNS(ns, 'text');
      containerLabel.setAttribute('x', cx + 8);
      containerLabel.setAttribute('y', cy + 16);
      containerLabel.setAttribute('fill', '#7aa0c4');
      containerLabel.setAttribute('font-size', '10');
      containerLabel.setAttribute('font-weight', '600');
      containerLabel.textContent = '📋 Section 卡片区';
      containerNode.appendChild(containerLabel);

      // clipPath：防止卡片溢出容器
      var clipId = 'section-card-clip';
      var defs = document.querySelector('svg defs') || (function() {
        var d = document.createElementNS(ns, 'defs');
        document.getElementById('canvas').insertBefore(d, document.getElementById('canvas').firstChild);
        return d;
      })();
      var oldClip = document.getElementById(clipId);
      if (oldClip) oldClip.remove();
      var clip = document.createElementNS(ns, 'clipPath');
      clip.setAttribute('id', clipId);
      var clipRect = document.createElementNS(ns, 'rect');
      clipRect.setAttribute('x', cx);
      clipRect.setAttribute('y', cy + 20);
      clipRect.setAttribute('width', cw);
      clipRect.setAttribute('height', ch - 20);
      clip.appendChild(clipRect);
      defs.appendChild(clip);

      var svg = document.getElementById('canvas');
      if (svg) svg.appendChild(containerNode);

      nodeById['node_section_card_container'] = {
        id: 'node_section_card_container',
        x: cx,
        y: cy,
        width: cw,
        height: ch,
      ...
      bindNodeEvents(containerNode, 'node_section_card_container');
      hideContainsEdges();
    }
    createSectionCardContainer();
      ...
      for (var i = 0; i < simDef.rules.length; i++) {
        ...
        for (var k = 0; k < emitted.length; k++) {
          ...
          if (typeof window.__simEventListener__ === 'function') {
            try { window.__simEventListener__(emitted[k], payload); } catch(e) {}
          }
          var cascade = processEvent(emitted[k], payload);
    ...
    function animateSimEdges(traceList) {
      ...
      for (var k = traceList.length - 1; k >= 0; k--) {
        if (traceList[k].conditionMet) {
          latestTrace = traceList[k];
          break;
        }
      }
      if (latestTrace) {
        spawnMessageParticles(latestTrace);
      }
    }
    ...
    var messageHistory = [];
    var msgFlowNode = null;
    var msgContainerIdCounter = 0;
    ...
    function ensureParticleLayer() {
      ...
      if (!particleLayer) {
        var ns = 'http://www.w3.org/2000/svg';
        particleLayer = document.createElementNS(ns, 'g');
        particleLayer.setAttribute('id', 'particle-layer');
        particleLayer.setAttribute('style', 'pointer-events: none;');
        var firstNode = svg.querySelector('.node');
        if (firstNode && firstNode.parentNode) {
          firstNode.parentNode.insertBefore(particleLayer, firstNode);
        } else {
    ...
    // 沿贝塞尔曲线生成点（二次贝塞尔）
    function pointOnQuadBezier(p0, p1, p2, t) {
      var mt = 1 - t;
      return {
    ...
    // 生成结构化消息文本（模拟 AAM 输出）
    function generateMessageText(ruleId, eventName) {
      var prefixes = ['!', '...', '「', '『', '【'];
      var middles = ['...', '……', '···', '... ...'];
      var suffixes = ['[summary]', '← sum', '✓', '📝', '✎'];
      var prefix = prefixes[Math.floor(Math.random() * prefixes.length)];
      var middle = middles[Math.floor(Math.random() * middles.length)];
      var suffix = suffixes[Math.floor(Math.random() * suffixes.length)];
      var shortEvent = eventName.length > 10 ? eventName.slice(0, 8) + '..' : eventName;
      return prefix + middle + suffix + ' ' + shortEvent;
    }

    // 创建消息流画布容器节点
    function ensureMsgFlowNode() {
      if (msgFlowNode) return msgFlowNode;
      var svg = document.getElementById('canvas');
      if (!svg) return null;
      var ns = 'http://www.w3.org/2000/svg';

      var nodeId = 'msg_flow_node';
      var nodeWidth = 170;
      var nodeHeight = 580;
      var nodeX = 1020;
      var nodeY = 40;
      ...
      g.setAttribute('class', 'node msg-flow-node');
      ...
      rect.setAttribute('fill', 'rgba(22, 33, 62, 0.95)');
      ...
      titleText.setAttribute('class', 'node-label msg-flow-title');
      titleText.setAttribute('x', nodeX + nodeWidth / 2);
      titleText.setAttribute('y', nodeY + 24);
      titleText.setAttribute('text-anchor', 'middle');
      titleText.setAttribute('fill', '#4fc3f7');
      titleText.setAttribute('style', 'font-size: 14px; font-weight: 600;');
      titleText.textContent = '📝 消息流';
      g.appendChild(titleText);

      var headerLine = document.createElementNS(ns, 'line');
      headerLine.setAttribute('x1', nodeX + 10);
      headerLine.setAttribute('y1', nodeY + 36);
      headerLine.setAttribute('x2', nodeX + nodeWidth - 10);
      headerLine.setAttribute('y2', nodeY + 36);
      headerLine.setAttribute('stroke', '#1f2a4d');
      headerLine.setAttribute('stroke-width', '1');
      g.appendChild(headerLine);

      var fo = document.createElementNS(ns, 'foreignObject');
      fo.setAttribute('x', nodeX + 10);
      fo.setAttribute('y', nodeY + 44);
      fo.setAttribute('width', nodeWidth - 20);
      fo.setAttribute('height', nodeHeight - 54);
      fo.setAttribute('class', 'msg-flow-fo');

      var div = document.createElement('div');
      div.setAttribute('xmlns', 'http://www.w3.org/1999/xhtml');
      div.className = 'msg-flow-list';
      div.style.cssText = 'width:100%;height:100%;overflow-y:scroll;color:#e6e6e6;font-size:11px;font-family:system-ui,sans-serif;pointer-events:none;';
      fo.appendChild(div);
      g.appendChild(fo);

      // foreignObject 上的鼠标事件全部穿透到父 <g>（让拖拽正常工作）
      // 但 wheel 事件保留，用于滚动消息列表
      fo.addEventListener('mousedown', function(e) {
        e.stopPropagation();
        // 手动在 <g> 上触发拖拽
      ...
      var clearBtn = document.createElementNS(ns, 'text');
      clearBtn.setAttribute('class', 'msg-flow-clear');
      clearBtn.setAttribute('x', nodeX + nodeWidth - 20);
      clearBtn.setAttribute('y', nodeY + 22);
      clearBtn.setAttribute('text-anchor', 'end');
      clearBtn.setAttribute('fill', '#8b9bb4');
      clearBtn.setAttribute('style', 'font-size:11px;cursor:pointer;');
      ...
      var nodesLayer = svg.querySelector('.nodes-layer') || svg;
      ...
      g.addEventListener('mouseenter', function(e) { showTooltip(e, nodeId); });
      g.addEventListener('mousemove', moveTooltip);
      g.addEventListener('mouseleave', hideTooltip);
      ...
      g.addEventListener('click', function(e) {
        if (e.shiftKey) {
          ...
          clearMultiSelect();
          selectNode(nodeId);
        } else if (state.selectedId === nodeId) {
          clearSelection();
        } else {
          selectNode(nodeId);
        }
    ...
    // 添加消息到历史
    function addMessageToHistory(msgText, ruleId, eventName) {
      var now = new Date();
    ...
    // 更新消息流节点内容
    function updateMsgFlowNode() {
      var node = ensureMsgFlowNode();
      if (!node) return;
      var list = node.querySelector('.msg-flow-list');
      if (!list) return;
      if (messageHistory.length === 0) {
        list.innerHTML = '<div style="color:#5a6a88;font-size:11px;padding:8px;text-align:center;">暂无消息，点击"发送"按钮开始仿真</div>';
        return;
      }
      var html = '';
      for (var i = 0; i < messageHistory.length; i++) {
        var msg = messageHistory[i];
        html += '<div style="padding:6px 8px;border-bottom:1px solid #1f2a4d;margin-bottom:4px;">' +
          '<div style="color:#8b9bb4;font-size:10px;margin-bottom:2px;">' + msg.time + '</div>' +
          '<div style="color:#e6e6e6;font-size:11px;line-height:1.4;">' + escapeHtml(msg.text) + '</div>' +
          '<div style="color:#5a6a88;font-size:10px;margin-top:2px;">规则: ' + escapeHtml(msg.ruleId) + ' | 事件: ' + escapeHtml(msg.event) + '</div>' +
          '</div>';
      }
      list.innerHTML = html;
    }
    ...
    // 显示/定位消息流节点
    function toggleMsgFlowPanel() {
      var node = ensureMsgFlowNode();
      if (!node) return;
      var shape = node.querySelector('[data-shape="true"]');
      if (!shape) return;
      var svg = document.getElementById('canvas');
      if (!svg) return;
      var x = parseFloat(shape.getAttribute('x'));
      var y = parseFloat(shape.getAttribute('y'));
      var w = parseFloat(shape.getAttribute('width'));
      var h = parseFloat(shape.getAttribute('height'));
      var viewBox = svg.getAttribute('viewBox').split(' ').map(Number);
      var svgRect = svg.getBoundingClientRect();
      var scale = viewBox[2] / svgRect.width;
      var cx = (x + w / 2) / scale;
      var cy = (y + h / 2) / scale;
      var flashCount = 0;
    ...
    // 粒子到达终点 → 转为可拖拽容器，放在消息流面板内部（不遮挡卡片区）
    function convertParticleToContainer(particle) {
      var ns = 'http://www.w3.org/2000/svg';
      var containerId = 'msg-container-' + (++msgContainerIdCounter);
      var g = document.createElementNS(ns, 'g');
      g.setAttribute('class', 'msg-container node');
      g.setAttribute('data-id', containerId);

      // 定位在消息流面板内部，水平不与卡片区重叠
      var panelX = 1020, panelY = 40, panelW = 170;
      var containerW = 150, containerH = 28;
      var slot = (msgContainerIdCounter - 1) % 16;
      var msgX = panelX + 10;
      var msgY = panelY + 46 + slot * 34;

      var rect = document.createElementNS(ns, 'rect');
      rect.setAttribute('data-shape', 'true');
      rect.setAttribute('x', msgX);
      rect.setAttribute('y', msgY);
      rect.setAttribute('width', containerW);
      rect.setAttribute('height', containerH);
      rect.setAttribute('fill', particle.color);
      ...
      text.setAttribute('x', msgX + containerW / 2);
      text.setAttribute('y', msgY + 18);
      text.setAttribute('text-anchor', 'middle');
      text.setAttribute('fill', '#ffffff');
      text.setAttribute('style', 'font-size:10px;font-weight:500;pointer-events:none;');
      var msgTextEl = particle.el.querySelector('.msg-text');
      var raw = msgTextEl ? msgTextEl.textContent : '';
      if (raw.length > 22) raw = raw.substring(0, 20) + '..';
      ...
      requestAnimationFrame(function() { g.style.opacity = '0.85'; });

      // 超过 16 个时最旧的淡出移除
      var containers = layer ? layer.querySelectorAll('.msg-container') : [];
      if (containers.length > 16) {
        var oldest = containers[0];
        oldest.style.opacity = '0';
        setTimeout(function() { if (oldest.parentNode) oldest.parentNode.removeChild(oldest); }, 300);
      }

      // 拖拽支持（满足 data-shape=true 可拖拽约束）
      g.style.cursor = 'grab';
      g.addEventListener('mousedown', function(e) {
        if (e.button !== 0) return;
        e.stopPropagation();
        ...
        var scaleX = viewBox[2] / svgRect.width;
        var scaleY = viewBox[3] / svgRect.height;
        var mouseX = (e.clientX - svgRect.left) * scaleX + viewBox[0];
        var mouseY = (e.clientY - svgRect.top) * scaleY + viewBox[1];
        var r = g.querySelector('rect[data-shape="true"]');
        var nodeX = r ? parseFloat(r.getAttribute('x')) : msgX;
        var nodeY = r ? parseFloat(r.getAttribute('y')) : msgY;
        state.dragging = containerId;
        state.dragOffset = { x: mouseX - nodeX, y: mouseY - nodeY };
        g.style.cursor = 'grabbing';
        document.body.style.userSelect = 'none';
      });
    ...
    function spawnMessageParticles(traceItem) {
      var layer = ensureParticleLayer();
      if (!layer) return;

      var ruleId = traceItem.ruleId;
      var simEdges = document.querySelectorAll('.sim-edge[data-rule="' + ruleId + '"]');
      if (simEdges.length === 0) return;

      var ns = 'http://www.w3.org/2000/svg';
      var colors = ['#4fc3f7', '#81c784', '#ffb74d', '#ba68c8', '#f06292', '#4dd0e1'];
      var color = colors[Math.floor(Math.random() * colors.length)];

      for (var i = 0; i < simEdges.length; i++) {
        (function(edge, idx) {
          var path = edge.querySelector('.sim-edge-path');
          if (!path) return;
          ...
          var p0 = { x: parseFloat(parts[0]), y: parseFloat(parts[1]) };
          var p1 = { x: parseFloat(parts[2]), y: parseFloat(parts[3]) };
          var p2 = { x: parseFloat(parts[4]), y: parseFloat(parts[5]) };

          var msgText = generateMessageText(ruleId, traceItem.event);

          var g = document.createElementNS(ns, 'g');
          g.setAttribute('class', 'msg-particle');
          g.style.opacity = '0';

          var bubble = document.createElementNS(ns, 'rect');
          bubble.setAttribute('class', 'msg-bubble');
          bubble.setAttribute('width', '120');
          ...
          var text = document.createElementNS(ns, 'text');
          text.setAttribute('class', 'msg-text');
          ...
          addMessageToHistory(msgText, ruleId, traceItem.event);
          ...
          g.addEventListener('mousedown', function(e) {
            ...
            var scaleX = viewBox[2] / svgRect.width;
            var scaleY = viewBox[3] / svgRect.height;
            particle.dragStartX = (e.clientX - svgRect.left) * scaleX;
            particle.dragStartY = (e.clientY - svgRect.top) * scaleY;
            var currentPos = pointOnQuadBezier(particle.p0, particle.p1, particle.p2, particle.t);
            particle.offsetX = particle.dragStartX - currentPos.x;
            particle.offsetY = particle.dragStartY - currentPos.y;
            g.style.cursor = 'grabbing';
    ...
    function tickParticles() {
      ...
      for (var i = particles.length - 1; i >= 0; i--) {
        ...
        p.t += p.speed;
        if (p.t >= 1) {
          convertParticleToContainer(p);
          particles.splice(i, 1);
          continue;
        }

        var pos = pointOnQuadBezier(p.p0, p.p1, p.p2, p.t);
        p.el.setAttribute('transform', 'translate(' + (pos.x - 60) + ', ' + (pos.y - 13) + ')');
        active = true;
      }
    ...
    document.addEventListener('mousemove', function(e) {
      for (var i = 0; i < particles.length; i++) {
        ...
        var mouseX = (e.clientX - svgRect.left) * scaleX;
        var mouseY = (e.clientY - svgRect.top) * scaleY;
        var posX = mouseX - p.offsetX;
        var posY = mouseY - p.offsetY;
        p.el.setAttribute('transform', 'translate(' + (posX - 60) + ', ' + (posY - 13) + ')');
      }
    });
    ...
    document.addEventListener('mouseup', function(e) {
      for (var i = particles.length - 1; i >= 0; i--) {
        ...
        var mouseX = (e.clientX - svgRect.left) * scaleX;
        var mouseY = (e.clientY - svgRect.top) * scaleY;
        var finalX = mouseX - p.offsetX;
        var finalY = mouseY - p.offsetY;
        fadeOutParticle(p);
        particles.splice(i, 1);
      }
    });

    // ==== Section 卡片动画（干净重写 v2） ====
    // 设计原则：
    // 1. 绝对坐标定位（rect x/y），不用 transform 做布局
    // 2. 先算布局再统一更新 DOM
    // 3. 新卡片从底部淡入上移，淘汰卡片左滑淡出
    // 4. 折叠/展开：rect height + text y 同步动画
    // 5. 容器高度固定，不调用 resizeParentToFitChildren
    // 6. transform 只用于动画偏移，动画结束后清除

    var prevSectionSnapshot = '';
    var CARD_PAD_X = 6;
    var CARD_PAD_Y = 26;
    var CARD_GAP = 5;
    var CARD_H_ACTIVE = 50;
    var CARD_H_FOLDED = 26;

    function updateSectionVisuals() {
      var containerNode = document.querySelector('.node[data-id="node_section_card_container"]');
      if (!containerNode) return;

      var cx = sectionCardLayout.cx;
      var cy = sectionCardLayout.cy;
      var cw = sectionCardLayout.cw;

      var sections = (simState && simState.sections) ? simState.sections : [];
      var ns = 'http://www.w3.org/2000/svg';

      // 快照对比（含 messages/tokens 变化）
      var currentSnapshot = sections.map(function(s) {
        return s.id + ':' + s.status + ':' + (s.messages || 0) + ':' + (s.tokens || 0);
      }).join('|');
      var snapshotChanged = currentSnapshot !== prevSectionSnapshot;
      prevSectionSnapshot = currentSnapshot;

      var cardX = cx + CARD_PAD_X;
      var cardW = cw - CARD_PAD_X * 2;

      // 收集现有卡片
      var existingMap = {};
      containerNode.querySelectorAll('.section-card').forEach(function(card) {
        existingMap[card.getAttribute('data-sec-id')] = card;
      });

      // 第一遍：计算每张卡片的目标布局
      var yCursor = cy + CARD_PAD_Y;
      var layouts = {};
      for (var i = 0; i < sections.length; i++) {
        var sec = sections[i];
        var isFolded = sec.status === 'folded';
        var h = isFolded ? CARD_H_FOLDED : CARD_H_ACTIVE;
        layouts[sec.id] = { y: yCursor, h: h, folded: isFolded, sec: sec };
        yCursor += h + CARD_GAP;
      }
      ...
      for (var j = 0; j < sections.length; j++) {
        ...
        var sid = sec2.id;
        keptIds[sid] = true;
        var layout = layouts[sid];
        var existing = existingMap[sid];

        if (existing) {
          var wasFolded = existing.classList.contains('folded');
          if (wasFolded !== layout.folded && snapshotChanged) {
            // 状态变化：折叠 ↔ 展开
            var oldH = wasFolded ? CARD_H_FOLDED : CARD_H_ACTIVE;
            existing.classList.toggle('folded', layout.folded);
            rebuildCardContent(existing, sec2, layout, cardX, cardW, ns);
            animateFoldTransition(existing, oldH, layout.h, layout.y);
          } else {
            // 仅更新内容和位置
            updateCardContent(existing, sec2, layout, cardX, cardW);
            positionCardRect(existing, cardX, layout.y, cardW, layout.h);
          }
        } else {
          // 新卡片
          var g = createSectionCard(sec2, layout, cardX, cardW, ns);
          containerNode.appendChild(g);
          newCards.push(g);
        }
      }
      ...
      Object.keys(existingMap).forEach(function(id) {
        ...
        animateCardSlideOut(existingMap[id], cardW);
      });

      // 新卡片淡入
      newCards.forEach(function(card) { animateCardFadeIn(card); });
    }

    // 创建单张卡片
    function createSectionCard(sec, layout, cardX, cardW, ns) {
      var g = document.createElementNS(ns, 'g');
      g.setAttribute('class', 'section-card' + (layout.folded ? ' folded' : ''));
      g.setAttribute('data-sec-id', sec.id);
      g.setAttribute('opacity', '0');
      g.setAttribute('clip-path', 'url(#section-card-clip)');
      ...
      rect.setAttribute('fill', layout.folded ? '#2d2d5a' : '#1e3a5f');
      rect.setAttribute('stroke', layout.folded ? '#5a4a8a' : '#3a5a8a');
      rect.setAttribute('stroke-width', 1);
      g.appendChild(rect);

      appendCardContent(g, sec, layout, cardX, cardW, ns);
      return g;
    }

    // 追加卡片内容（title + body/badge）
    function appendCardContent(g, sec, layout, cardX, cardW, ns) {
      var titleText = document.createElementNS(ns, 'text');
      titleText.setAttribute('class', 'card-title');
      titleText.setAttribute('x', cardX + 10);
      titleText.setAttribute('y', layout.y + 17);
      titleText.setAttribute('fill', layout.folded ? '#b39ddb' : '#ffffff');
      titleText.setAttribute('font-size', '11');
      titleText.setAttribute('font-weight', layout.folded ? '600' : '500');
      var titleStr = sec.summary || sec.id;
      if (titleStr.length > 42) titleStr = titleStr.substring(0, 40) + '...';
      titleText.textContent = (layout.folded ? '📦 ' : '📝 ') + titleStr;
      g.appendChild(titleText);

      if (!layout.folded) {
        var bodyText = document.createElementNS(ns, 'text');
        bodyText.setAttribute('class', 'body-text');
        bodyText.setAttribute('x', cardX + 10);
        bodyText.setAttribute('y', layout.y + 34);
        bodyText.setAttribute('fill', '#8b9bb4');
        bodyText.setAttribute('font-size', '10');
        bodyText.textContent = '... ' + (sec.messages || 0) + ' msg · ' + (sec.tokens || 0) + ' tok ...';
        g.appendChild(bodyText);
      } else {
        var foldBadge = document.createElementNS(ns, 'text');
        foldBadge.setAttribute('class', 'fold-badge');
        foldBadge.setAttribute('x', cardX + cardW - 8);
        foldBadge.setAttribute('y', layout.y + 17);
        foldBadge.setAttribute('text-anchor', 'end');
        foldBadge.setAttribute('fill', '#8b9bb4');
        foldBadge.setAttribute('font-size', '10');
        foldBadge.textContent = '✂ folded';
        g.appendChild(foldBadge);
      }
    }

    // 重建卡片内容（状态变化时：先清空再追加）
    function rebuildCardContent(g, sec, layout, cardX, cardW, ns) {
      g.querySelectorAll('text').forEach(function(t) { t.remove(); });
      appendCardContent(g, sec, layout, cardX, cardW, ns);
    }

    // 更新卡片内容文本（无状态变化时）
    function updateCardContent(g, sec, layout, cardX, cardW) {
      var title = g.querySelector('.card-title');
      if (title) {
        var titleStr = sec.summary || sec.id;
        if (titleStr.length > 42) titleStr = titleStr.substring(0, 40) + '...';
        title.textContent = (layout.folded ? '📦 ' : '📝 ') + titleStr;
        title.setAttribute('y', layout.y + 17);
      }
      var body = g.querySelector('.body-text');
      if (body && !layout.folded) {
        body.textContent = '... ' + (sec.messages || 0) + ' msg · ' + (sec.tokens || 0) + ' tok ...';
        body.setAttribute('y', layout.y + 34);
      }
      var badge = g.querySelector('.fold-badge');
      if (badge && layout.folded) {
        badge.setAttribute('y', layout.y + 17);
      }
    }

    // 设置 rect 位置
    function positionCardRect(g, x, y, w, h) {
    ...
    function animateFoldTransition(g, fromH, toH, baseY) {
      var rect = g.querySelector('rect');
      if (!rect) return;
      ...
      function tick(now) {
        ...
        rect.setAttribute('height', fromH + (toH - fromH) * ease);
    ...
    // 淘汰卡片左滑+淡出
    function animateCardSlideOut(g, cardW) {
      var startTime = performance.now();
      var duration = 400;
    ...
    function setupSectionAnimation() {
      var simSectionNode = document.querySelector('.node[data-id="node_section_queue"]');
      if (!simSectionNode) return;
      updateSectionVisuals();
    }
