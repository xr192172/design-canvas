 ...
 *
 * 启用（serve 启动时）：
 *   CAMERA_GUARD=1   # 开启实时判定打印
 *
 * 叠加在现有 enableCameraFromEnv 的 onEvent 回调之上——该回调已负责 SSE 推送，
...
/** 是否开启 console 实时判定打印（读取 CAMERA_GUARD 环境变量）。 */
export function judgeGuardEnabled(): boolean {
  return process.env.CAMERA_GUARD === '1';
}
...
export function judgeGuardLog(ev: TSEvent): void {
  ...
  console.error(
    `[camera-guard][${ts}] ✗ 偏差 rule=${v.rule} probe=${v.probe} source=${ev.source}\n` +
      `    ${v.reason}\n` +
