...
| **H** L5 反向提取 / 生成层 | ✅ P1 全 | [derive_anim_flow.ts](file:///d:/project_develop/design-canvas/src/tools/derive_anim_flow.ts)（MCP 工具 `derive_anim_flow`）把 derive_detail_chain 的调用链 + CFG 自动转成 `animations_v2.flows`：chain flow（L4 函数绑定，调用链相邻对 handler 绑定调用方）+ branch flow（L3 条件分支，CFG 条件→分支路由，末位 else 兜底，mock_values 从 shapes.in 数值/布尔极端对生成）+ errors（L4.5 异常，源码扫描注入 handler.errors：Go return ErrXxx→expected / panic→unexpected，JS throw / Py raise→unexpected）+ cross flow（序号 15：基于 cache.db 跨文件调用边，L4 绑定越出本文件到宿主，max_cross 参数默认 3）；幂等（只重建 `${node_id}__flow_` 前缀 flows，保留手写）；依赖先跑 derive_detail_chain | tests/tools/derive_anim_flow.test.ts 13 用例 + 全量 496 通过 |
...
3. **D3 注入回放**：进料口编辑 JSON / 预设异常场景 → 回放暴露问题 ✅（工具侧：`inject_replay`——注入值替代 mock_results 静态推演：classifyError 异常分类 + pickBranch 分支求值 + shapes.out ajv 质检；preset 从 errors 声明自适应构造注入值；纯只读零风险。进料口 UI 编辑面板留作后续交互糖）
